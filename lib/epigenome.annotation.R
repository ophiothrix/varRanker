...
epigenome.annotation <- function(variants, tissue.id) {
  ...
  annos <- list.files("./data/ENCODE", pattern = tissue.id, full.names = T)
  ...
  ## Add "distance to summit" columns for narrow peak marks
  dist.to.summit <- matrix(1000, nrow = length(variants), ncol = length(grep("narrow", annos)))
  colnames(dist.to.summit) <- paste(gsub(paste0(".*", tissue.id, "-(.*)\\..*"), "\\1", annos[grep("narrow", annos)]), "dist.to.summit", sep = ".")
  
  ## Combine with the rest of the annotation table
  anno.matrix <- cbind(anno.matrix, dist.to.summit)
  
  ## Set NA values for FC to 1
  ...
  for (anno in annos) {
    ...
    print(anno.name)
    ## Import a peaks file
    if (length(grep("broad", anno)) == 1) { ## assign correct column names for broad peak file
      ...
      if (length(grep("narrow", anno)) == 1) { ## assign correct column names for narrow peak file
        ...
        ## make a table of narrow peak coordinates
    ...
    if (length(grep("narrow", anno)) == 1) {
      ...
      anno.matrix[queryHits(olaps), paste0(anno.name, ".", "dist.to.summit")] <- abs(start(variants[queryHits(olaps)]) - peaks.meta$summit.coord[subjectHits(olaps)])
    }
    ## Add binary yes / no annotation for each mark
    anno.matrix[queryHits(olaps), paste0(anno.name, ".bin")] <- 1
  }
  # anno.matrix$regulatory <- variants$regulatory
  # saveRDS(anno.matrix, "./data/annotated.play.training.set.rds")
  ...
  state <- list.files("./data/ENCODE/chromHMM.calls", paste0(tissue.id, "_18_core"), full.names = T)
  print(state)
  ...
  ## Add annotation for different chromHMM classes for a given tissue
  state <- list.files("./data/ENCODE/IDEAS", tissue.id, full.names = T)
  print(state)
  ...
  if (all(1:length(variants) %in% queryHits(olaps))) {
  	...
  	variants$IDEAS.state <- as.numeric(NA)
