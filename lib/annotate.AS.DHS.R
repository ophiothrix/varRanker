annotate.AS.DHS <- function(variant.file.id, tissue.id) {
	...
	variants <- GRanges(read.table(paste0("./data/snps.multicell.bySample/snps.multicell.", variant.file.id, ".bed"), col.names = c("chr", "start", "end", "AS_DHS"), stringsAsFactors = F))
	...
	if (!all(variants %in% all.as.dhs)) {
		warning("Some of the tisue-specific variants are not found in the global file")
	}
