## Currently distance to summit is set to 1kb if the variant doesn't fall into a range for a given mark. Instead of setting an arbitrary "negative" distance. Set it at the distance to the nearest summit

variants <- readRDS("./cache/E120.annotated.HSMM.variants.rds")
variants <- readRDS("./cache/E120.annotated.HSMM.negative.set.rds")
tissue.id <- "E120"
...
epigenome.annotation <- function(variants, tissue.id) {
    ...
    dist.to.summit <- matrix(NA, nrow = length(variants), ncol = length(grep("narrow", annos)))
    ...
    anno.matrix <- cbind(anno.matrix, dist.to.summit, imputed.anno.matrix)
    ...
    for (anno in annos) {
        ...
        if (length(grep("narrow", anno)) == 1) {
            ...
            anno.matrix[dist.to.sum$queryHits, paste0(anno.name, ".", "dist.to.summit")] <- dist.to.sum$distance
        }
